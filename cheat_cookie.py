...
import keyring  # encryption
import pyaes  # AES Block Cipher algorithm implementation
from pbkdf2 import (
    PBKDF2,
)  # This module implements the password-based key derivation function, PBKDF2, specified in RSA PKCS#5 v2.0.

__doc__ = "Documentation goes here..."
...
def create_local_copy(cookie_file):
    """
    build a copy of the sqlite cookie for our use
    :return: new absolute filename
    """
    ...
    if os.path.exists(cookie_file):
        ...
        raise BrowserCookieError("Failed to find cookies: " + cookie_file)
...
## Taken from chrome cookies code
def crypt_unprotect_data(
    ...
    if not ctypes.windll.crypt32.CryptUnprotectData(
        ...
        raise RuntimeError("DPAPI decoding failure")
...
class Chrome:
    def __init__(self, cookie_file=None, domain_name="", key_file=None):
        ...
        self.iv = b" " * 16
        self.length = 16
        # below is domain name to filter cookies by
        ...
        if sys.platform == "darwin":
            ...
            iterations = 1003  # count of pbkdf2 iterations for mac os
            self.key = PBKDF2(my_pass, self.salt, iterations=iterations).read(
                self.length
            )
            ...
            # cookie extraction from APPDATA
            #  \\ is required to prevent unicode error in windows
            cookie_file = (
                cookie_file
            ...
            raise BrowserCookieError("OS not supported")

        if isinstance(cookie_file, list):
            if not cookie_file:
                raise BrowserCookieError("Failed to fetch Chrome cookie")
    ...
    def load(self):
        """
        Loadint the sqlite cookies in the form of a cookiejar
        """
    ...
    def _decrypt(self, value, encrypted_value):
        ...
        if sys.platform == "win32":
            ...
            except RuntimeError:  # DPAPI decoding failed
                if not self.key:
                    raise RuntimeError(
                        "decrypt cipher text with DPAPI and no AES key resulted in failure"
                    )
...
class Firefox:
    def __init__(self, cookie_file=None, domain_name=""):
        ...
        self.tmp_cookie_file = create_local_copy(cookie_file)
        # sessions running are in sessionstore.js
        self.session_file = os.path.join(
            os.path.dirname(cookie_file), "sessionstore.js"
        )
        self.session_file_lz4 = os.path.join(
            os.path.dirname(cookie_file), "sessionstore-backups", "recovery.jsonlz4"
        )
    ...
    def find_cookie_file():
        ...
        cookie_files = (
            glob.glob(
                os.path.join(
                    Firefox.get_default_profile(user_data_path), "cookies.sqlite"
                )
            )
            or cookie_files
        )
        ...
        if cookie_files:
            ...
            raise BrowserCookieError("Failed to fetch Firefox cookie")
    ...
    def __add_session_cookies(self, cj):
        ...
        try:
            ...
            print("Error firefox session JSON:", str(e))
        else:
    ...
    def __add_session_cookies_lz4(self, cj):
        ...
        try:
            ...
            print("Error firefox session JSON LZ4:", str(e))
        else:
...
def create_cookie(host, path, secure, expires, name, value):
    """this can be used to create a cookie"""
